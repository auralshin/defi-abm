...
class AMMAgent(Agent):
    ...
    Supports configurable pricing curves and hooks for observing swap/deposit/withdraw events.
    """
    ...
    def _charge_fee(self, amount_in: float) -> Tuple[float, float, float]:
        """
        Split the input into user amount, LP fee, and protocol fee.
        """
    ...
    def swap_x_for_y(self, amount_x_in: float) -> float:
        """
        Perform a swap from token X to token Y.
        Updates reserves and applies fees accordingly.
        """
        if amount_x_in <= 0:
            return 0.0
    ...
    def withdraw_liquidity(self, provider: Agent, lp_tokens: float) -> Tuple[float, float]:
        """
        Burn LP tokens and withdraw corresponding reserves.
        """
    ...
    def step(self):
        """
        No internal logic—AMMs react to external events (e.g. swaps).
        """
        pass
